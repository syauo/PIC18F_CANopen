...
*******************************************************************************/
   ...
   //晶振频率，可以使用PLLx4模式（如：8MHz 晶振设置 PLLx4 后得到 32MHz）
...
*******************************************************************************/
    ...
    * 如果位于RAM中，应使用'const'来代替'rom'）。对象字典中的 ROM 变量必须追加 ATTR_ROM 属性。
    * 如果 base 属性为 ATTR_RW 或 ATTR_WO，则可通过 CAN 网络更改变量（写入 PIC18Fxxx 微控制器
    * 中的闪存空间）
    */
   
   #define ROM             rom

   //default qualifiers and types   默认的限定符和类型
   // 在 PIC18F 上，本地变量如果是静态的话更快
   ...
   // 用以将 CAN-ID 和 RTR 位写入位于 CO_TXCAN 或 CO_RXCAN 数组的标识符内
   // 标识符与硬件寄存器对齐，RTR 位必须为 0 或 1
